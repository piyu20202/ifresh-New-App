...
import com.ifresh.customerr.helper.PaymentModelClass;
...
public class CheckoutActivity extends AppCompatActivity implements OnMapReadyCallback, PaymentResultListener {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        storePrefrence = new StorePrefrence(CheckoutActivity.this);
        ...
        //buy default message set here
        //msgtxt.setText(storePrefrence.getString("checkout_deliveryChargeMessage"));
        ...
        if(total<Constant.SETTING_MINIMUM_AMOUNT_FOR_FREE_DELIVERY)
        {
            ...
            msgtxt.setTextColor(CheckoutActivity.this.getResources().getColor(R.color.red));
        }
        else if(total>=Constant.SETTING_MINIMUM_AMOUNT_FOR_FREE_DELIVERY)
        {
            ...
            msgtxt.setTextColor(CheckoutActivity.this.getResources().getColor(R.color.colorPrimary));
        }
            ...
            public void onClick(View view) {
                if (chWallet.getTag().equals("false")) {
                    ...
                    if (Constant.WALLET_BALANCE >= total)
                    {
                        ...
                        usedBalance = total;
                        //Log.d("usedbalance", ""+usedBalance);
                        //subtotal = subtotal + 100;

                        /*if(subtotal <= Constant.SETTING_MINIMUM_AMOUNT_FOR_FREE_DELIVERY-1) {
                            subtotal = subtotal + storePrefrence.getInt("delivery_chrge");
                        }*/
        ...
        switch (view.getId()) {
            ...
            case R.id.tvUpdate:
                if (ApiConfig.isGPSEnable(CheckoutActivity.this))
                    ...
                    startActivity(new Intent(CheckoutActivity.this, MapsActivity.class));
                else
    ...
    public void PlaceOrderProcess() {
        ...
        sendparams.put("version_code", String.valueOf(storePrefrence.getInt("version_code")));
        ...
        final String address = session.getData(Session.KEY_ADDRESS) + ", " + session.getData(Session.KEY_AREA_N) + ", " + session.getData(Session.KEY_CITY_N) + ", " + session.getData(Session.KEY_PINCODE) + ", Deliver to " + label;
        if (!pCode.isEmpty()) {
        ...
        System.out.println("=====paramsss " + sendparams.toString());
        Log.d("params", sendparams.toString());
    ...
    protected void onResume() {
        ...
        String address = session.getData(Session.KEY_ADDRESS) + ",<br>"
                + session.getData(Session.KEY_AREA_N)
                + ", " + session.getData(Session.KEY_CITY_N)

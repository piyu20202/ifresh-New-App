...
import com.ifresh.customerr.helper.TextDrawable
import com.ifresh.customerr.model.CityName
import java.util.*


class CityAdapter(var mContext: Context, var cityArrList: ArrayList<CityName>) : BaseAdapter()
{
    private val inflater: LayoutInflater = mContext.getSystemService(Context.LAYOUT_INFLATER_SERVICE) as LayoutInflater

    override fun getCount(): Int {
        return cityArrList.size
    }
    ...
    override fun getView(position: Int, convertView: View?, parent: ViewGroup?): View
    {
        ...
        holder.cityname.text = cityArrList.get(position).city_name
        holder.cityname.tag = cityArrList.get(position).city_id

        val d = TextDrawable(mContext)
        d.text = cityArrList.get(position).city_name?.substring(0, 1)
    ...
    private class ViewHolder(row: View?) {
        val cityname:TextView = row?.findViewById(R.id.text_itemlist_spinner) as TextView

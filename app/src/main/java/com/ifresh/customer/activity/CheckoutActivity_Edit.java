...
public class CheckoutActivity_Edit extends AppCompatActivity implements OnMapReadyCallback, PaymentResultListener
{
    ...
    Boolean is_address_save=false, is_default_address_save=false, is_order_cancel=false,is_order_cancel_already=false;
            ...
            public void onClick(View v)
            {
                ...
                handler.postDelayed(new Runnable()
                {
                    ...
                    public void run() {
                        ...
                        if(is_order_cancel_already)
                        {
                            //order already cancel
                            prgLoading.setVisibility(View.GONE);
                            Toast.makeText(ctx,"Your order can not be placed because it has been processed.", Toast.LENGTH_SHORT).show();
                        }
                        else
                        {
                                    ...
                                    public void onSuccess(boolean result, String response) {
                                        if (result) {
                                            try {
                                                ...
                                                {
                                                    //storePrefrence.setString("order_id", object.getString("order_id"));
                                                    for(int i=0; i<variantIdList.length();i++)
                                                    {
                                                        String frproductvarId = variantIdList.get(i).toString();
                                                        String productId=prodvIdList.get(i).toString();
                                                        String frproductId=frenpid.get(i).toString();
                                                        if(storePrefrence.getString("order_id").equalsIgnoreCase("0"))
                                                        {
                                                            //databaseHelper.UpdateOrderData_edit(frproductvarId, productId,frproductId , object.getString("order_id"),  "0");
                                                            databaseHelper.UpdateOrderData_edit_checkout(frproductvarId, productId,frproductId , "0", object.getString("order_id"),  "0");
                                                        }
                                                        else{
                                                            databaseHelper.UpdateOrderData_edit_checkout(frproductvarId, productId,frproductId , storePrefrence.getString("order_id"), object.getString("order_id"),  "0");
    ...
    {
        Map<String, String> params = new HashMap<String, String>();
        params.put(Constant.ID, storePrefrence.getString("order_id"));
    ...
    private String getDateToSend(){
        Log.d("Constant",""+ Constant.DELIVERY_DAY_AFTER_ORDER);
